...
struct Configuration {
  ...
  static let LINE_WIDTH:CGFloat = 22.0
...
func DegreesToRads (value:Double) -> Double {
  return value * M_PI / 180.0
}

func RadsToDegrees (value:Double) -> Double {
...
class PlusCircularSlider: UIControl {
  ...
  override init(frame: CGRect) {
    ...
    let radiusMultiplier = CGFloat(1.2)
  ...
  override func draw(_ rect: CGRect){
    ...
    imageCtx!.setLineWidth(Configuration.LINE_WIDTH)
    ...
    imageCtx!.addArc(center: imageCenter,
                     radius: radius,
                     startAngle: 0,
                     endAngle: CGFloat(DegreesToRads(value: Double(angle))),
                     clockwise: true)
  ...
  func drawTheHandle(ctx:CGContext){
    ...
    ctx.strokeEllipse(in: CGRect(x: handleCenter.x, y: handleCenter.y, width: Configuration.LINE_WIDTH, height: Configuration.LINE_WIDTH));
...
//    let hotSpots = [67, 74, 80, 86, 92, 98, 104, 115]
  ...
  func moveHandle(lastPoint:CGPoint){
    ...
    let circleYPosition = self.frame.size.height * circleYPositionMultiplier
    //Circle center
    let centerPoint = CGPoint(x: 207 - Configuration.LINE_WIDTH/2.0, y: circleYPosition - Configuration.LINE_WIDTH / 2);
...
//    let hotSpots = [67, 74, 80, 86, 92, 98, 104, 115]
  ...
  func pointFromAngle(angleInt:Int)->CGPoint{
    ...
    let y = round(Double(radius) * sin(DegreesToRads(value: Double(-angleInt)))) + Double(centerPoint.y)
    let x = round(Double(radius) * -(cos(DegreesToRads(value: Double(-angleInt))))) + Double(centerPoint.x)
    result.y = CGFloat(y)
    result.x = CGFloat(x)
  ...
  func pointFromAngleForSpots(angleInt:Int)->CGPoint{
    ...
    let circleYPosition = self.frame.size.height * circleYPositionMultiplier
    //Circle center
    let centerPoint = CGPoint(x: 207 - 3 / 2 , y: circleYPosition - 3 / 2);
  ...
  func pointFromAngleForAnimation(angleInt:Int)->CGPoint{
    ...
    let centerPoint = CGPoint(x: 207 - 1 / 2 , y: circleYPosition - 1 / 2);
  ...
  func AngleFromNorth(p1:CGPoint , p2:CGPoint , flipped:Bool) -> Double {
    ...
    result = RadsToDegrees(value: radians)
